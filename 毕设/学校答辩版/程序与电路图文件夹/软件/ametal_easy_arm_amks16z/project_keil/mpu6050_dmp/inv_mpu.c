...
#define i2c_write   am_mpu_write_len
#define i2c_read    am_mpu_read_len
#define delay_ms    am_mdelay
...
#define log_e       PrintChar        /* 自定义用于打印输出 */
#define log_i       PrintChar        /* 自定义用于打印输出 */
#define PrintChar   AM_DBG_INFO      /* 自定义用于打印输出 */
#define u8          uint8_t
...
//#define min(a,b) ((a<b)?a:b) /* ametal 中已定义 */
...
int mpu_set_sensors(unsigned char sensors)
{
    ...
    if (sensors & INV_XYZ_GYRO)  /* 以陀螺仪 x 轴 PLL为参考 */
        data = INV_CLK_PLL;
    else if (sensors)
        data = 0;
    else
    ...
    if (sensors && (sensors != INV_XYZ_ACCEL))
        ...
        mpu_set_int_latched(0); /* 不锁住中断电平状态 */
    ...
    if (sensors & INV_XYZ_COMPASS) {
        ...
        data = AKM_POWER_DOWN;
        user_ctrl &= ~BIT_AUX_IF_EN;   /* 没有使用主模式 */
    }
    if (st.chip_cfg.dmp_on)
 ...
 *  @param[in]  length  Length of one FIFO packet,这里传进来的值为0
...
#define LOAD_CHUNK  (16)     /* 这里可以一次性加载小一点就可以了 */
...
/*************************************************************************** 
  MPU6050的DMP库添加与应用程序相关的代码的实现
***************************************************************************/

/**
 * \name q30格式,long转float时的除数
 * @{ 
 */
...
void delay_ms1(unsigned long time)
{
    if (time / 100) {
...
/* mpu6050,dmp初始化 */
unsigned char am_mpu_dmp_init(am_mpu6050_handle_t handle)
{
    ...
    {     
      ...
      if (!mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL)) {
          ...
          PrintChar("mpu_set_sensor error ......\r\n");
      }
      if (!mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL)) {   /* mpu_configure_fifo         */
          PrintChar("mpu_configure_fifo complete ......\r\n");     
      } else {                   
          PrintChar("mpu_configure_fifo error ......\r\n");     
      }
      ...
      if (!dmp_load_motion_driver_firmware()) {                   /* dmp_load_motion_driver_firmvare */
          ...
          PrintChar("dmp_load_motion_driver_firmware error......\n");
      }
          ;
      if (!dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation))) {  /* dmp_set_orientation */
         PrintChar("dmp_set_orientation complete ......\n");
      }else {
         PrintChar("dmp_set_orientation error ......\n");
      }
           ;
      if (!dmp_enable_feature(DMP_FEATURE_6X_LP_QUAT     | DMP_FEATURE_TAP |
          ...
          PrintChar("dmp_enable_feature error ......\n");         
      }
      
      if (!dmp_set_fifo_rate(DEFAULT_MPU_HZ)) {                                        /* dmp_set_fifo_rate */
          PrintChar("dmp_set_fifo_rate complete ......\n");
      } else {
          PrintChar("dmp_set_fifo_rate complete error ......\n");
      }
      if (!run_self_test()) {
          PrintChar("dmp_run_self_test come complete ......\n");
      } else {
          PrintChar("dmp_run_self_test come error ......\n");
      }
      if (!mpu_set_dmp_state(1)) {  
          PrintChar("mpu_set_dmp_state complete ......\n");
      } else {
          PrintChar("mpu_set_dmp_state error ......\n");
      }
          
    }
   
    /* MPU6050中断引脚的初始化为高电平极性激活 */    
    mpu_set_int_level(0); 
    
    return 0;
}
/* 得到dmp处理后的数据(注意,本函数需要比较多堆栈,局部变量有点多) */
unsigned char am_mpu_dmp_get_data(am_mpu6050_handle_t  handle, 
                                  short int  *gyro, 
                                  short int  *accel, 
                                  float  *pitch_roll_yaw)
{
    ...
    if(sensors & INV_WXYZ_QUAT) {
        ...
        /* 计算得到俯仰角/横滚角/航向角 */
        pitch_roll_yaw[0] = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3;                                    /* pitch */
        pitch_roll_yaw[1] = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3;    /* roll  */
        pitch_roll_yaw[2] = atan2(2*(q1*q2 + q0*q3),q0*q0+q1*q1-q2*q2-q3*q3) * 57.3;                  /* yaw   */ 
    } else  {        
        return 2;
    }
    return 0;
