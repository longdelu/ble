...
int am_uart_int_receive(am_uart_int_handle_t  handle,
                        ...
                        uint32_t              nbytes);
/**
 * \brief UART数据接收（中断模式），在一定时间内接收不到数据超时退出
 *
 * \param[in] handle  : UART（中断模式）标准服务操作句柄
 * \param[in] p_rxbuf : 接收数据缓冲区
 * \param[in] nbytes  : 待接收数据的个数
 * \param[in] time_ms : 等待超时的时间，单位为 ms
 * 
 * \return AM_NO_WAIT : 超时不等待
 * \return 其它，成功接收数据的个数
 */
int am_uart_int_time_out_receive (am_uart_int_handle_t  handle,
                                  uint8_t              *p_rxbuf,
                                  uint32_t              nbytes,
                                  uint32_t             time_ms);
