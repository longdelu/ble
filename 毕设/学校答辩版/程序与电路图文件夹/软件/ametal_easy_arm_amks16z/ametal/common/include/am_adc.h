...
struct am_adc_drv_funcs {
    ...
    /** \brief 指向ADC中断连接函数 */
    int (*pfn_adc_connect) (void *p_drv);
 ...
 */
am_static_inline
int am_adc_connect (am_adc_handle_t handle)
{
    return handle->p_funcs->pfn_adc_connect(handle->p_drv);
}
 ...
 */
am_static_inline
uint32_t am_adc_vref_get (am_adc_handle_t handle, int chan)
{
