 ...
 *            - AM_UART_FLOWMODE_SET    : 设置流控模式, p_arg为 uint32_t指针类型（无，软，硬流控）
 *            - AM_UART_FLOWSTAT_RX_SET : 设置接收器状态，p_arg为 uint32_t指针类型（继续或停止传送）
 *            - AM_UART_FLOWSTAT_TX_GET : 获取发送器状态，p_arg为 uint32_t指针类型
