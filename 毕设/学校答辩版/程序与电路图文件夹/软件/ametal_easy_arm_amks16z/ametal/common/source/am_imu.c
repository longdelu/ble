...
{  
    ...
    */
    ...
    g_gyrofinal.X = temp1 / AM_IMU_FILTER_NUM;
    g_gyrofinal.X = temp2 / AM_IMU_FILTER_NUM;
    g_gyrofinal.X = temp3 / AM_IMU_FILTER_NUM;
    ...
    g_accfinal.X  = (float)((g_accbuf.X-g_accoffset.X) * 0.061f) * 0.0098f;        
    g_accfinal.Y  = (float)((g_accbuf.Y-g_accoffset.Y) * 0.061f) * 0.0098f;        
    g_accfinal.Z  = (float)((g_accbuf.Z-g_accoffset.Z) * 0.061f) * 0.0098f;    
...
void am_imu_ag_updata (float gx, float gy, float gz, float ax, float ay, float az)
{
   ...
   /*
    * 向量叉乘，得到机体坐标系的合加速茺的大小与方向, 因为MPU6050测量的数值不包括重力的
    ...
    */
     ...
     * 正好拿来纠正陀螺。（你可以自己拿东西想象一下）由于陀螺是对机体直接积分，所以对陀螺的纠正量会直接体现在对机体坐标系的纠正。
     */
    ex = (ay * vz - az * vy);                                               
    ey = (az * vx - ax * vz);
    ez = (ax * vy - ay * vx);
