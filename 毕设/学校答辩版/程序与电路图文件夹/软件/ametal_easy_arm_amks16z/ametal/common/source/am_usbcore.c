...
//USB设备描述符的定义
uint8_t DeviceDescriptor[0x12]=  //设备描述符为18字节
{
...
void UsbDisconnect(void)
{
 ...
 D12WriteByte(0x06); //设置模式的第一字节,断开软连接上拉电阻
 D12WriteByte(0x47); //设置模式的第二字节，CLKOUT时钟频率为6MHz
 DelayXms(1000);     //延迟1秒
}
////////////////////////End of function//////////////////////////////
...
#endif
 D12WriteCommand(D12_SET_MODE);  //写设置模式命令
 D12WriteByte(0x16); //设置模式的第一字节,连接上软连接上拉电阻
 ...
 if(D12ReadEndpointLastStatus(0)&0x20)
 {
  ...
  if((bmRequestType&0x80)==0x80)
  {
   ...
   switch((bmRequestType>>5)&0x03)
   {
    ...
    case 1:  //类请求
     #ifdef DEBUG0
      AM_DBG_INFO("USB类输入请求：\r\n");
     #endif
    break;
    
    case 2:  //厂商请求
     #ifdef DEBUG0
      AM_DBG_INFO("USB厂商输入请求：\r\n");
     #endif
    break;
    
    default: //未定义的请求。这里只显示一个报错信息。
     #ifdef DEBUG0
      AM_DBG_INFO("错误：未定义的输入请求。\r\n");
     #endif
    break;
   }
  }
  ...
  else //if(bmRequestType&0x80==0x80)之else
  {
   ...
   switch((bmRequestType>>5)&0x03)
   {
    case 0:  //标准请求
     ...
     switch(bRequest)
     {
      ...
      case SET_ADDRESS:  //设置地址
       #ifdef DEBUG0
        AM_DBG_INFO("设置地址。地址为：");
        AM_DBG_INFO("0x%x",wValue&0xFF); //显示所设置的地址
        AM_DBG_INFO("\r\n");
       #endif
    ...
    case 2:  //厂商请求
     #ifdef DEBUG0
      AM_DBG_INFO("USB厂商输出请求：\r\n");
     #endif
    break;
    
    default: //未定义的请求。这里只显示一个报错信息。
     #ifdef DEBUG0
      AM_DBG_INFO("错误：未定义的输出请求。\r\n");
     #endif
    break;
   }
  }
...
/**
 * \brief 设备中断处理函数 
 * \param[in] flag   : 中断标志位，值为0或1，0：中断未发生，1：中断已发生 
 * \return    无
 */
