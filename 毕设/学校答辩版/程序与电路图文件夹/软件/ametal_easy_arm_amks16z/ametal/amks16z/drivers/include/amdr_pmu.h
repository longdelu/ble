...
typedef struct amdr_pmu_devinfo {
    amhw_smc_t       *p_hw_smc;   /**< \brief 指向SMC寄存器块的指针  */
    amhw_pmc_t       *p_hw_pmc;   /**< \brief 指向PMC寄存器块的指针  */
    amhw_rcm_t       *p_hw_rcm;   /**< \brief 指向RCM寄存器块的指针  */
    amhw_llwu_t      *p_hw_llwu;  /**< \brief 指向LLWU寄存器块的指针 */
...
typedef enum amdr_pmu_mode {
    AMDR_PMU_MODE_RUN = 0,       /**< \brief 运行模式          */
    AMDR_PMU_MODE_VLPR,          /**< \brief 超低功耗运行模式  */
    AMDR_PMU_MODE_WAIT,          /**< \brief 等待模式          */
    AMDR_PMU_MODE_VLPW,          /**< \brief 超低功耗等待模式  */
    AMDR_PMU_MODE_STOP2,         /**< \brief 停止模式2         */
    AMDR_PMU_MODE_STOP1,         /**< \brief 停止模式1         */
    AMDR_PMU_MODE_STOP,          /**< \brief 正常停止模式      */
    AMDR_PMU_MODE_VLPS,          /**< \brief 超低功耗停止模式  */
    AMDR_PMU_MODE_LLS,           /**< \brief 低漏电停止模式    */
    AMDR_PMU_MODE_VLLS3,         /**< \brief 超低漏电停止模式3 */
    AMDR_PMU_MODE_VLLS1,         /**< \brief 超低漏电停止模式1 */
    AMDR_PMU_MODE_VLLS0          /**< \brief 超低漏电停止模式0 */
} amdr_pmu_mode_t;

/** \brief 掉电报警电压阈值（报警电压大于复位电压） */
typedef enum amhw_pmu_warn_v_sel {
    AMDR_PMU_WARN_V1 = 0,              /**< \brief 选择报警阈值1(1.8v)\(2.7V) */
    AMDR_PMU_WARN_V2,                  /**< \brief 选择报警阈值2(1.9V)\(2.8V) */
    AMDR_PMU_WARN_V3,                  /**< \brief 选择报警阈值3(2.0V)\(2.9V) */
    AMDR_PMU_WARN_V4                   /**< \brief 选择报警阈值4(2.1V)\(3.0V) */
} amhw_pmu_warn_v_sel_t;

/** \brief 掉电复位电压阈值 */
typedef enum amdr_pmu_rest_v_sel {
    AMDR_PMU_REST_VL = 0,              /**< \brief 选择低阈值检测(1.6V ) */
    AMDR_PMU_REST_VH                   /**< \brief 选择高阈值检测(2.56V) */
} amdr_pmu_rest_v_sel_t;

/** \brief PMU 掉电信息配置 */
typedef struct amdr_pmu_brownout_info {
    amhw_pmu_warn_v_sel_t  warn_v;      /**< \brief 报警电压选择 */
    amdr_pmu_rest_v_sel_t  rest_v;      /**< \brief 复位电压选择 */
    am_pfnvoid_t           p_callfunc;  /**< \brief 报警回调函数 */
    void                  *p_arg;       /**< \brief 回调参数     */
    bool_t                 enable;      /**< \brief 是否使能     */
} amdr_pmu_brownout_info_t;

/** \brief LLWU唤醒源信息
 *  
 * 应使用函数amdr_pmu_llwu_src_x_mkinfo()初始化该结构体
 */
...
/**
 * \brief 获取系统模式
 * \retval AMDR_PMU_MODE_RUN  : 运行模式
 * \retval AMDR_PMU_MODE_VLPR : 低功耗运行模式
 */
