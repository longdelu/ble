...
/**
 *  brief amhw_ftfa_cmd_launch函数编译后数据拷贝
 *  到RAM需要的空间大小 （以半字 16bit 计）
 */
#define CMD_LAUNCH_FUNC_SIZE    (40)

static uint16_t amhw_ftfa_cmd_launch_ram_context[CMD_LAUNCH_FUNC_SIZE];

static uint32_t (* amhw_ftfa_cmd_launch_ram) (amhw_ftfa_t *);
 ...
 */
static uint32_t amhw_ftfa_cmd_launch (amhw_ftfa_t *p_hw_ftfa)
{
    /** 等待之前的操作完成或者超时 */
    while (!(p_hw_ftfa->fstat & AMHW_FTFA_CCIF)) {
        ;
    }
...
void amhw_ftfa_func_copy (void)
{
    ...
    p = (uint16_t *)(((uint32_t)&amhw_ftfa_cmd_launch) & (~0x1));

    /** 将函数的指令拷贝到RAM中 */
    for (i = 0; i < CMD_LAUNCH_FUNC_SIZE; i++) {
        amhw_ftfa_cmd_launch_ram_context[i] = *p;
        p++;
    }

    /** 将RAM中函数的起始地址给 amhw_ftfa_cmd_launch_ram
     *  由于Cortex M0+ 只支持 thumb指令
     *  所以 amhw_ftfa_cmd_launch_ram 的最低位为 1
     */
    amhw_ftfa_cmd_launch_ram
        = (uint32_t (*) (amhw_ftfa_t *))       /** 强制转化为函数指针 */
          (
           (uint32_t)                          /** 强制转化为整型便于运算 */
           amhw_ftfa_cmd_launch_ram_context | 0x01
           );
}
/**
 *  \brief 发送判断扇区是否为全1命令
...
uint32_t amhw_ftfa_1s_read (amhw_ftfa_t *p_hw_ftfa,
                            ...
                            uint8_t      margin)
{
    p_hw_ftfa->fccob0 = amhw_ftfa_rd1sec;
    ...
    return amhw_ftfa_cmd_launch_ram(p_hw_ftfa);
}
...
uint32_t amhw_ftfa_program_check (amhw_ftfa_t *p_hw_ftfa,
                                 ...
                                 uint8_t      margin)
{
    p_hw_ftfa->fccob0 = amhw_ftfa_pgmchk;
...
{
    uint8_t status;
    p_hw_ftfa->fccob0 = amhw_ftfa_rdrsrc;
    p_hw_ftfa->fccob1 = (addr >> 16) & 0xff;
    p_hw_ftfa->fccob2 = (addr >> 8 ) & 0xff;
    p_hw_ftfa->fccob3 = (addr >> 0 ) & 0xff;
    p_hw_ftfa->fccob8 = resrc_select;

    status = amhw_ftfa_cmd_launch_ram(p_hw_ftfa);
...
uint32_t amhw_ftfa_word_program (amhw_ftfa_t *p_hw_ftfa,
                                 ...
                                 uint32_t     word)
{
    p_hw_ftfa->fccob0 = amhw_ftfa_pgm4;
    p_hw_ftfa->fccob1 = (addr >> 16) & 0xff;
    p_hw_ftfa->fccob2 = (addr >> 8 ) & 0xff;
    p_hw_ftfa->fccob3 = (addr >> 0 ) & 0xff;
...
uint32_t amhw_ftfa_sector_erase (amhw_ftfa_t *p_hw_ftfa,
                                 uint32_t     addr)
{
    p_hw_ftfa->fccob0 = amhw_ftfa_ersscr;
    p_hw_ftfa->fccob1 = (addr >> 16) & 0xff;
    p_hw_ftfa->fccob2 = (addr >> 8 ) & 0xff;
    p_hw_ftfa->fccob3 = (addr >> 0 ) & 0xff;
...
uint32_t amhw_ftfa_all_blocks_1s_read (amhw_ftfa_t *p_hw_ftfa,
                                       uint8_t      margin)
{
    p_hw_ftfa->fccob0 = amhw_ftfa_rd1all;
...
{
    uint8_t status;
    p_hw_ftfa->fccob0 = amhw_ftfa_rdonce;
...
uint32_t amhw_ftfa_once_program (amhw_ftfa_t *p_hw_ftfa,
                                 ...
                                 uint32_t     value)
{
    p_hw_ftfa->fccob0 = amhw_ftfa_pgmonce;
    p_hw_ftfa->fccob1 = index;
...
{
    p_hw_ftfa->fccob0 = amhw_ftfa_ersall;
...
uint32_t amhw_ftfa_backdoor_access_key_verify (amhw_ftfa_t *p_hw_ftfa,
                                               uint8_t      key[8])
{
    p_hw_ftfa->fccob0 = amhw_ftfa_vfykey;
