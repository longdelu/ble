...
#define AMHW_MCM_ESFC       (1UL << 16)  /**< \brief FLASH忙延时标志位      */

#define AMHW_MCM_CFCC       (1UL << 10)  /**< \brief FLASH高速缓存清除位    */

#define AMHW_MCM_ARB        (1UL << 9 )  /**< \brief 仲裁方式选择位         */

#define AMHW_MCM_CPOWOI     (1UL << 2 )  /**< \brief 中断唤醒计算操作使能位 */

#define AMHW_MCM_CPOACK     (1UL << 1 )  /**< \brief 计算操作应答标志位     */

#define AMHW_MCM_CPOREQ     (1UL << 1 )  /**< \brief 计算操作请求标志位     */

/**
 *  \name 仲裁模式
 *  @{
 */
#define FIX_PRIO            (0)          /**< \brief 固定优先权 */
#define ROUND_ROBIN         (1)          /**< \brief 循环模式   */
/**
 *  @}
 */
 ...
 *  @{
 */
#define SEECULATE_CODE      (0)         /**< \brief 预测缓存代码       */
#define SPECULATE_CODE_DATA (1)         /**< \brief 预测缓存代码和数据 */
#define SPECULATE_NONE      (2)         /**< \brief 无预测缓存         */
/**
 *  @}
 */
...
typedef struct amhw_mcm {
    ...
    __I  uint32_t reserve1[51];     /**< \brief 保留                        */
