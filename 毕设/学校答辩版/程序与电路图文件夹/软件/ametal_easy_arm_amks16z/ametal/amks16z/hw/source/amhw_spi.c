 ...
 * - 1.00 15-11-02  liqing, first implementation.
 * \endinternal
 */
...
{
    if ( irq_flags & AMHW_SPI_IRQ_MATCH) {       /* 数据匹配中断       */
        p_hw_spi->c2 |= 0x80;
    }
    if ( irq_flags & AMHW_SPI_IRQ_R_MODDEF) {    /* 接收或多主机中断   */
        p_hw_spi->c1 |= 0x80;
    }
    if ( irq_flags & AMHW_SPI_IRQ_T) {           /* 发送完成中断       */
        p_hw_spi->c1 |= 0x20;
    }
}
