...
/**
 * \brief PIT通道ID,用于操作具体的某个通道，有效范围：0 ~ (AMHW_PIT_CHANNELS_NUM - 1)
 */
...
#define AMHW_PIT_MSC_MASK     (0x3u)            /**< \brief 状态和控制寄存器掩码 */
#define AMHW_PIT_MSC_MDIS     (1UL << 1)        /**< \brief 模块时钟禁能 */
#define AMHW_PIT_MSC_FRZ      (1UL << 0)        /**< \brief 调试模式下定时器暂停 */
...
#define AMHW_PIT_LTH_MASK     (0xFFFFFFFFu)   /**< \brief 生命周期定时器高位部分 */
#define AMHW_PIT_LTL_MASK     (0xFFFFFFFFu)   /**< \brief 生命周期定时器低位部分 */
 ...
 *
 * \param[in] p_hw_pit : 指向PIT寄存器块的指针
 * \param[in] chan     : 通道ID，使用宏#AMHW_PIT_CH(n)
 * \param[in] val      : 需要写入的值
 *
 * \return 无
 */
...
#define AMHW_PIT_TCTL_MASK          (0x7u)           /**< \brief 模数掩码 */
#define AMHW_PIT_TCTL_CHAIN_MODE    (1UL << 2)       /**< \brief 链模式 */
#define AMHW_PIT_TCTL_TI_EN         (1UL << 1)       /**< \brief 定时器中断使能 */
#define AMHW_PIT_TCTL_TMR_EN        (1UL << 0)       /**< \brief 定时器使能 */
 ...
 * \param[in] p_hw_pit : 指向PIT寄存器块的指针
 * \param[in] chan     : 通道ID，使用宏#AMHW_PIT_CH(n)
 * \param[in] flags    : AMHW_PIT_TCTL_* 宏值或多个
 *                       AMHW_PIT_TCTL_*宏的 或（OR）值(#AMHW_PIT_TCTL_TI_EN)
 * \return 无
 */
am_static_inline
...
#define AMHW_PIT_TMR_FLAG        (1UL << 0)             /**< \brief 定时器使能 */
