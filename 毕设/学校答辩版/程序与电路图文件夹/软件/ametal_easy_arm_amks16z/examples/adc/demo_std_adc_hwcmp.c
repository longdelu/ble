...
/**
 * \file
 * \brief ADC例程，硬件触发转换，进行转换结果比较，DMA传输转换结果，通过标准接口实现
 *
 * - 操作步骤：
 ...
 *
 * - 实验现象：
 *   1. 当vol<=0.83或者 Vol>=1.67v, 串口输出该电压采样值。
 *
 * \par 源代码
 * \snippet demo_std_adc_hwcmp.c src_std_adc_hwcmp
 *
 * \internal
...
/**
 * \addtogroup demo_if_std_adc_hwcmp
 * \copydoc demo_std_adc_hwcmp.c
 */
...
const amdr_adc_cmp_info_t g_adc0_cmp_info = {
    AMDR_ADC_CMP_EN,             /* 开启比较 */
    AMDR_ADC_CMP_MODE_6,         /* 比较条件为 "res <= a" 或者  "res >= b" */
    ((1 << 12) - 1) / 3,         /* 参考的电压的1/3点 */
    (((1 << 12) - 1) << 1) / 3,  /* 参考的电压的2/3点 */
};
/** \brief ADC 设备信息 （硬件触发，有比较无硬件平均）*/
const amdr_adc_devinfo_t g_adc0_devinfo_hwcmp =
{
...
int main (void)
{
    ...
    adc0_handle = amdr_adc_init(&g_adc0_dev, &g_adc0_devinfo_hwcmp);

    printf("The ADC CMP Demo\r\n");
