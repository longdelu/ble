...
/**
 * \file
 * \brief ADC例程，硬件触发转换，硬件平均转换结果，DMA传输转换结果，通过标准接口实现
 *
 * - 操作步骤：
 ...
 *
 * \par 源代码
 * \snippet demo_std_adc_hwavg.c src_std_adc_hwavg
 *
 * \internal
...
/**
 * \addtogroup demo_if_std_adc_hwavg
 * \copydoc demo_std_adc_hwavg.c
 */
...
const amdr_adc_avg_info_t g_adc0_avg_info = {
    AMDR_ADC_AVG_EN,            /* 开启硬件平均配置 */
    AMHW_ADC_AVGS_32,       /* 每次平均32个采样值 */
};

/** \brief ADC 设备信息 （硬件触发，无比较有硬件平均）*/
const amdr_adc_devinfo_t g_adc0_devinfo_hwavg =
{
    ...
    &g_adc0_avg_info,              /**< \brief 使用硬件平均功能       */
    NULL,                          /**< \brief 不使用比较功能       */
};
...
int main (void)
{
    ...
    adc0_handle = amdr_adc_init(&g_adc0_dev, &g_adc0_devinfo_hwavg);

    printf("The ADC AVG Demo\r\n");
